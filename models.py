...
class Channel(db.Model):
    ...
    tvg_id: Mapped[str] = mapped_column(String(100))
    tvg_name: Mapped[str] = mapped_column(String(200))
...
class SearchHistory(db.Model):
    ...
    category_type: Mapped[str] = mapped_column(String(20))  # live, movie, series, all
    playlist_id: Mapped[int] = mapped_column(Integer, ForeignKey('m3u_playlist.id'))
...
class UserSession(db.Model):
    ...
    current_playlist_id: Mapped[int] = mapped_column(Integer, ForeignKey('m3u_playlist.id'))
    last_activity: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
    preferences: Mapped[str] = mapped_column(Text)  # JSON string for user preferences
